...
| Dependency shape | Parent composes exactly the children it depends on via `#[Embed]` → nothing to add. Dependency set comes from rows → `UriTagInterface::fromAssoc`. A child that is itself cacheable must advertise the same shared key as its writers announce, or the parent rebuilds from a stale child |
