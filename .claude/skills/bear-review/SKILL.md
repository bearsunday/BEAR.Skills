...
#### 🛡️ とりあえずエスケープ教（過剰エスケープ症候群）

「エスケープしておけば安全」という思考停止。どこで何のためにエスケープするか理解していない。
...
class UserController
{
    public function save(array $input): void
    {
        ...
        $bio = strip_tags(htmlspecialchars($input['bio']));

        // DBに保存（エスケープ済みデータが入る）
        $this->query->add($name, $email, $bio);  // &lt;script&gt; がDBに入る
    }

    public function show(int $id): array
    {
        $user = $this->query->item($id);

        // 出力時にもエスケープ（二重エスケープ）
        return [
            'name' => htmlspecialchars($user->name),  // &amp;lt;script&amp;gt;
        ];
    }
}
...
$sql = "SELECT * FROM users WHERE name = '" .
    htmlspecialchars(mysqli_real_escape_string($conn,
...
class UserController
{
    public function save(array $input): void
    {
        // 入力: バリデーションのみ、エスケープしない
        $name = $input['name'];   // 生データを保存
        $email = $input['email'];
        $bio = $input['bio'];

        // DB: プレースホルダーでSQLインジェクション対策
        $this->query->add($name, $email, $bio);
    ...
    public function show(int $id): array
    {
        ...
        // JSON API: エスケープ不要（json_encodeが処理）
        return [
            'name' => $user->name,  // そのまま返す
            'bio' => $user->bio,
        ];
    }
}

// HTML出力時のみエスケープ（テンプレートエンジンが自動で行う）
// Twig: {{ user.name }}  ← 自動エスケープ
// Blade: {{ $user->name }}  ← 自動エスケープ
```
...
// 症状3: 文脈無視のエスケープ
$url = htmlspecialchars($userInput);  // URLなのにHTMLエスケープ
header("Location: $url");  // 意味なし

// 症状4: SQLにHTMLエスケープ
$sql = "... WHERE name = '" . htmlspecialchars($name) . "'";
// SQLインジェクション対策になってない

// 症状5: JSONにHTMLエスケープ
echo json_encode(['name' => htmlspecialchars($name)]);
// 不要なエスケープ
```
...
| 出力先 | 対策 | 方法 |
|--------|------|------|
| HTML | XSS | `htmlspecialchars()` or テンプレートエンジン |
| SQL | SQLi | プレースホルダー（プリペアドステートメント） |
| URL | URLi | `urlencode()` / `rawurlencode()` |
| JSON | - | `json_encode()`（自動処理） |
| シェル | コマンドインジェクション | `escapeshellarg()` |
| 正規表現 | ReDoS | `preg_quote()` |

**覚えておくべきこと:**
- **入力時**: バリデーション（形式チェック）、エスケープはしない
- **保存時**: 生データを保存（プレースホルダーでSQLi対策）
- **出力時**: 出力先に応じたエスケープ（できれば自動化）
- **原則**: データは常に生で持ち、出力直前に変換
