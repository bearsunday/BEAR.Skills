---
name: bear-migration
short-description: Migrate legacy PHP apps to BEAR.Sunday safely
summary: Reference-first migration workflow for porting legacy PHP/Symfony/EC-CUBE style applications to BEAR.Sunday without inventing unapproved infrastructure.
description: Use when migrating or porting an existing PHP application to BEAR.Sunday, converting legacy/Symfony/EC-CUBE routes/controllers/templates/services to resources, planning migration waves, or when the user says 移植, マイグレーション, legacy migration, route migration, Symfony to BEAR, EC-CUBE to BEAR, or asks how to avoid custom framework code during a BEAR.Sunday migration.
---
